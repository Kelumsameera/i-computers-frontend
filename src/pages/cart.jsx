  ...
  return (
    <div className="w-full flex flex-col items-center p-5">
      {cart.map((item, index) => (
        ...
        >
          <h1 className="lg:hidden w-full overflow-hidden h-5 absolute top-0">
            {item.name}
          </h1>
          <div className="h-full flex flex-col">
            ...
            {item.labelledPrice > item.price && (
              <h2 className=" lg:hidden text-secondary/80 line-through decoration-[gold] decoration-2 mr-2 text-sm">
                LKR. {item.labelledPrice.toFixed(2)}
              </h2>
            )}

            <h2 className="lg:hidden text-sm text-accent font-semibold mt-2">
          ...
          <div className="hidden lg:flex flex-col justify-center pl-4 w-[300px]">
            ...
            {item.labelledPrice > item.price && (
              <h2 className="text-secondary/80 line-through decoration-[gold] decoration-2 mr-2 text-lg">
                LKR. {item.labelledPrice.toFixed(2)}
              </h2>
            )}
